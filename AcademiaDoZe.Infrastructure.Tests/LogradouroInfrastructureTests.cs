...
public class LogradouroInfrastructureTests : TestBase
{
    ...
    {
        var cep = GerarCep();
        var logradouroResult = Logradouro.Criar(0, cep, "Gabriel", "Francisco de Sousa", "SQLite", "SC", "Brasil");
    ...
    {
        var cep = GerarCep();
        var logradouro = Logradouro.Criar(0, cep, "Gabriel", "Francisco de Sousa", "SQLite", "SC", "Brasil").Value!;
        ...
        Assert.Equal("Gabriel", inserido.Nome);
    ...
    public async Task Logradouro_Atualizar_Sucesso()
    {
        ...
        var logradouroAtualizado = Logradouro.Criar(logradouro.Id, novoCep, "Gabriel", "Francisco de Sousa", "SQLite", "SC", "Brasil").Value!;
        var resultado = await _repository.Atualizar(logradouroAtualizado);
        Assert.NotNull(resultado);
        Assert.Equal("Gabriel", resultado.Nome);
        Assert.Equal("Francisco de Sousa", resultado.Bairro);
        Assert.Equal("SQLite", resultado.Cidade);
        var noBanco = await _repository.ObterPorId(logradouro.Id);
        Assert.NotNull(noBanco);
        Assert.Equal("Gabriel", noBanco.Nome);
    }
    [Fact]
    public async Task Logradouro_Atualizar_LancaExcecaoQuandoInexistente()
    {
        var cep = GerarCep();
        var logradouroInexistente = Logradouro.Criar(999999, cep, "Gabriel", "Francisco de Sousa", "SQLite", "SC", "Brasil").Value!;
